...
import {
  Fn, If, clamp, float, vec2, vec3, vec4, abs, dot, exp, length, max, mix, smoothstep, texture,
} from 'three/tsl';
...
export function shapeTauAndRim(sky, U, map, { glow = false, rimHalo = false } = {}) {
  /* Wrapped because `If` needs a TSL stack, which only exists inside an `Fn`;
     packing tau into .w keeps it to one field evaluation for both outputs. */
  const packed = Fn(() => {
  ...
  return { tau: packed.w, rim: packed.xyz };
}
