...
import {
  Fn, float, floor, vec2, vec3, cos, sin, dot, fract, mix, pow, smoothstep,
} from 'three/tsl';
import { fbm3o2, fbm3o4, hash3, FBM2_NORM, FBM4_NORM } from './noise.js';

const TURN = Math.PI * 2;
const WRAP_H = 4096.0;

/* Independent slice of the noise domain for the mottle field */
...
function fanField(sky, U) {
  ...
  const axis = vec2(cos(U.uFanAngle), sin(U.uFanAngle));
  const cosIn = cos(U.uFanHalf.min(Math.PI)).toVar();
  const cosOut = cos(U.uFanHalf.add(U.uFanEdge).min(Math.PI)).min(cosIn.sub(1e-4));
  ...
  const env = ext.mul(float(1).sub(smoothstep(tipLo, len, r))).toVar();
...
function shadowTrans(dirHat, r, U, count) {
  ...
  for (let k = 0; k < count; k++) {
    ...
    const band = smoothstep(cOut, cIn, dot(dirHat, vec2(cos(ang), sin(ang))));
...
export function buildShadowFanNodes(skyU, U, opts = {}) {
  ...
  const continuum = Fn(() => {
    ...
    let amp = f.body.mul(lit).mul(limb).mul(U.uFanLum);
    if (useMottle) {
    ...
    if (knots > 0) amp = amp.mul(shadowTrans(f.dirHat, f.r, U, knots));
    return tint.mul(amp);
  })();
...
export const SHADOWFAN_DEFAULTS = {
  mottleOn: true,
  apex: [0.74, 0.78], angle: -2.15, half: 0.42, edge: 0.16,
  len: 0.5, fade: 0.45, litR: 0.14, falloff: 1.0,
  lum: 0.5, tint: [0.62, 0.76, 1.0], warm: [1.0, 0.82, 0.62],
  warmR: 0.05, warmAmt: 0.45, limb: 0.5,
  freq: 12.0, aniso: 0.25, threshold: 0.4, softness: 0.3, floor: 0.28,
  mottle: 0.35, motFreq: 5.5, morphRate: 0.05,
  shadowCount: 5, shadow: 0.85, shadowW: 0.13, shadowIn: 0.03,
