...
export function buildPlanetaryNodes(skyU, U, { fliers = true } = {}) {
  const line = Fn(() => {
    ...
    const R = R0.mul(float(1).sub(U.uPnWaist.mul(pinch)))
      .mul(ang.sub(0.5).mul(U.uPnWobble).add(1.0)).max(1e-3).toVar();
    ...
    const colO = shellColumn(b, R.sub(U.uPnSep).max(1e-3), U.uPnThick).toVar();
    const colH = shellColumn(b, R.add(U.uPnSep), U.uPnThick).toVar();
    ...
    /* Thermal-pulse onion skins. Darkening only, so stacked rings can never
       blow the shell up. */
    const dec = exp(b.div(U.uPnRingFade.max(1e-3)).negate())
      .mul(U.uPnRing.clamp(0.0, 1.0)).toVar();
    const rings = float(1).sub(dec.mul(
      float(0.5).sub(cos(b.mul(U.uPnRingFreq).add(U.uPnRingPhase)).mul(0.5)),
    )).toVar();
    ...
    const mottle = mix(float(1).sub(U.uPnMottle.clamp(0.0, 1.0)), float(1.0),
      smoothstep(0.3, 0.7, ang));
    const dens = breakW.mul(rings).mul(torus).mul(mottle).toVar();
    ...
    const he = float(1).sub(smoothstep(0.0, 1.0, hx)).toVar();
    /* Rings reach the halo too: the outermost pulses are where a real AGB halo
       shows them, and `dec` already fades them with radius. */
    const halo = he.mul(he).mul(mix(float(0.3), float(1.0), stria)).mul(rings)
      .mul(U.uPnHalo).toVar();

    const ha = colH.mul(dens).mul(U.uPnHa).add(halo).toVar();
    const oiii = colO.mul(dens).mul(U.uPnOiii).add(halo.mul(U.uPnHaloOiii));
...
export const PLANETARY_DEFAULTS = {
  center: [0.62, 0.52], rot: 0.55, aspect: 1.35, waist: 0.42, pinch: 2.2,
  radius: 0.155, expand: 0.000006, thick: 0.034, sep: 0.007, torus: 0.5,
  wobble: 0.14, motFreq: 7.0, mottle: 0.35,
  ring: 0.4, ringFreq: 190.0, ringPhase: 1.7, ringFade: 0.45,
  striaFreq: 7.0, striaAniso: 0.32, striaSharp: 2.6, striaEro: 0.5,
  cov: 0.8, breakup: 0.55,
  haloR: 3.4, halo: 0.1, haloOiii: 0.35,
  fliers: true, flier: 0.8, flierR: 1.28, flierSize: 0.016, flierHa: 0.9,
